...
MAX_REQUESTS = 35  # The number of eStreamRead calls to perform
run_time = 34
# Open first found T7
...
in_data =[]
with open('io/svhn/mode_'+str(mode)+'_io/chip_'+str(work_sel)+'_in.txt') as fp:
    for line in fp:
        in_data.append(line.rstrip())
fp.close()
print(in_data)
out_expected = []
with open('io/svhn/mode_' +str(mode)+'_io/chip_'+str(work_sel)+'_out.txt') as fp:
    for line in fp:
...
def reset_fifos():
    ...
    time.sleep(0.01) #Need to have at least 3 slow clock cycles for correct reset
...
for channel in range(3): #[0,0,0]:
    ...
    while i <= MAX_REQUESTS:
        ...
        time.sleep(1) #TODO This should be based on the per-en done language but its just simpler to over-provision runtime
        aData = ret[0]
