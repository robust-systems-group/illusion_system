...
MAX_REQUESTS = 6  # The number of eStreamRead calls to perform
# Open first found T7
...
in_data =[]
with open('io/mnist/mode_'+str(mode)+'_io/chip_'+str(work_sel)+'_in.txt') as fp:
    for line in fp:
        in_data.append(line.rstrip())
fp.close()
print(in_data)
out_expected = []
with open('io/mnist/mode_' +str(mode)+'_io/chip_'+str(work_sel)+'_out.txt') as fp:
    for line in fp:
...
for channel in range(3):
    ...
    print(len(data_out))
    print("Errors:")
    #print([int(data_out[i] != out_expected[i]) for i in range(len(data_out))])
