...
extra = datasets.SVHN(root=path, split='extra', download=True,
                       transform=transforms.Compose([
                           transforms.ToTensor(),
                           transforms.Normalize((0.5,0.5,0.5,), (0.5,0.5,0.5,))
                       ]))
train = datasets.SVHN(root=path, split='train', download=True,
                       transform=transforms.Compose([
                           transforms.ToTensor(),
                           transforms.Normalize((0.5,0.5,0.5,), (0.5,0.5,0.5,))
                       ]))

train_loader = DataLoader(ConcatDataset([train,extra]),
...
model = torch_modles.SVHN_Net(act_error_quant, act2_error_quant)
...
for epoch in range(30):
    ...
    if test_res['accuracy'] > best_result: 
        best_result = test_res['accuracy']
        print("Saving model")
        torch.save(model.state_dict(), './checkpoints/svhn_model_quant.pth')
