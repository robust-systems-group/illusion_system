...
int classify(int z, int print_mode) {
    ...
    for (i = 0; i < 12; i++) {
        conv2D_1filter(data.s.n1_conv1_out, N1_layer2_H + (i*1*1*6), N1_layer2_B[i], 6, 6, 1, 0, data.s.n1_conv2_out + (i*6*6));
    }
    for (i = 0; i < 12; i++) {
    ...
    if (high_low) { 
        for (i = 0; i < 12; i++) {
            conv2D_1filter(data.s.n1_conv2_out, N2_layer1_H + (i*3*3*12), N2_layer1_B[i], 12, 6, 3, 0, data.s.n2_conv1_out + (i*4*4));
        }
        for (i = 0; i < 24; i++) {
