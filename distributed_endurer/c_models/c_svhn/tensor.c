...
inline int32_t quant(int32_t a){
    //Quantized to q bits
    //m_in is number of dec bits for fp in
    //m_out is number of dec bits qRelu quantizes to
    if (a < 0){
        int32_t rup = a - BIAS; // round down 
        if (rup < -OMAX) return -IMAX; //return 2**frac -1
        else return (rup >> SHIFT); //bit shift back to m_out frac bits
    }
    else {
...
void add_bias(const int8_t *B, int32_t *C, int16_t *CO, int Z) {
    int z;
    for (z = 0; z <Z; z++) {
        CO[z] = (int16_t) ((C[z] + (B[z] << (SHIFT+2))) >> (SHIFT+2));
    }
}
...
int8_t conv1_out_raw[8*2*8*2];

void print_write(int8_t* O, int start, int index_8, int print_mode) {
    if (print_mode!=-1) return;
    int16_t* O2 = (int16_t*) O;
    printf("%d,%04x,%04x\n",
            0,
            start+(index_8/2),
            (uint16_t) O2[index_8/2]);
}

//__attribute__((optimize("unroll-loops")))
int conv2D_1filter_pad_stuck(int8_t *A, const int8_t *H, int8_t B, int IC, int X, int FX, int MPP, int8_t *O,
                             int8_t *conv1_forced_ones_, int8_t *conv1_forced_zeros_ , int start_address, int print_mode) {
    // H: Weights (Filter)
    // A: Input
    // O: Output
    // MPP: Max pool (power of 2):  only 2^MPP max pooling supported as no divider
    ...
    for (i = 0; i < X>>MPP; i++) 
        for (j = 0; j < X>>MPP; j++) {
            O[(X>>MPP)*i + j] = INT8_MIN ; // -128;
            O[(X>>MPP)*i + j] |= conv1_forced_zeros_[(X>>MPP)*i + j]; // stuck at 1
            O[(X>>MPP)*i + j] &= conv1_forced_ones_[(X>>MPP)*i + j]; // stuck at 0
            print_write(O,start_address,(X>>MPP)*i+j,print_mode);
        } 
    ...
    for (i = 0; i < X; i++){  
        ...
        for (j = 0; j < X; j++){  //Iterate through the input image
            ...
            // bn +  quant , maxpool   , no relu
            temp = (int8_t) quant(temp);
            if (temp<0) temp = 0; 
            //Max pool 
            idxo = (i >> MPP)*(X >> MPP) + (j >> MPP); 
            if (( (int8_t) temp) > O[idxo]) {
                O[idxo] = (int8_t) temp;
                O[idxo] |= conv1_forced_zeros_[idxo]; // stuck at 1
                O[idxo] &= conv1_forced_ones_[idxo];  // stuck at 0
                print_write(O,start_address,idxo,print_mode);
            }
            idxo++;
        }
    }
...
int conv2D_1filter_pad(int8_t *A, const int8_t *H, int8_t B, int IC, int X, int FX, int MPP, int8_t *O ) {
    ...
    for (i = 0; i < X; i++){  
        ...
        for (j = 0; j < X; j++){  //Iterate through the input image
            ...
            idxo = (i >> MPP)*(X >> MPP) + (j >> MPP); 
            // only bn +  quant , maxpool , no relu; assume fixed in SW to not require multiple writes to RRAM   
            conv1_out_raw[idxo] = (conv1_out_raw[idxo] > quant(temp)) ? conv1_out_raw[idxo] : (int8_t) quant(temp);
