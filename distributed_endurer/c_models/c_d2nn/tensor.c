...
//__attribute__((optimize("unroll-loops")))
int conv2D_1filter_stuck(int8_t *A, const int8_t *H, int8_t B, int IC, int X, int FX, int MPP, int8_t *O,
                         int8_t *n1_conv2_forced_ones_, int8_t *n1_conv2_forced_zeros_ ,
                         int8_t *n1_conv2_out_raw) {
    ...
    for (i=0;i<M;i++) for (j=0;j<M;j++) n1_conv2_out_raw[i*M+j] = 0;
    ...
    for (i = 0; i < M; i++){  
        for (j = 0; j < M; j++){  //Iterate through the input image
            ...
            //printf("\n %d \n", temp);
            //printf("%6d ", temp);
             // only bn +  quant , no maxpool    
            int8_t temp2 = quant(temp);
            // Cleaned out unnecessary if
            // Pre relu saved for high path
            if (n1_conv2_out_raw[i*M+j] < temp2) n1_conv2_out_raw[i*M+j] = temp2;
            // Removed: bit processing; should only be applied later to high path
            // Post relu saved for low path
            temp = n1_conv2_out_raw[i*M+j];
            if (temp<0) temp = 0; 
